document.addEventListener('DOMContentLoaded', () => {
  ...
  const cartSection = document.getElementById('cartSection');  // New: Reference to cart section
...
function renderCart() {
  ...
  // Note: saveCartToStorage() is now called earlier when cart is empty, or here if not empty
  if (cart.length > 0) {
    saveCartToStorage();
  }
}
...
// Checkout button handler (UPDATED)
if (checkoutBtn) {
  checkoutBtn.addEventListener('click', () => {
    ...
    if (appliedVoucherDiscount > calculatedSubtotal) {
      appliedVoucherDiscount = calculatedSubtotal;
    }
    
    const finalTotalAfterDiscount = Math.max(calculatedSubtotal - appliedVoucherDiscount, 0);
    
    // Save the FULL cart to a separate key
    localStorage.setItem('fullCartItems', JSON.stringify(cart));
    // Save checked items to another key
